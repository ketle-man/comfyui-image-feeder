...
- **播放控制**:
  ...
  - 👁 **Prev ON/OFF**: 在按钮下方显示/隐藏图像预览面板。每次执行时自动更新。
...
- **批量输出与自动缩放**:
  ...
  - 当循环末尾的批次剩余图像数量少于 `batch_size` 时，以最后一张图像重复填充（不回绕到开头）。
...
- 同一工作流中可放置多个 Image Feeder 节点，各节点独立运行互不干扰。
- 循环过程中若发生队列错误，运行按钮将自动恢复为可用状态。
